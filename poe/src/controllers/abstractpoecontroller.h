...
public:
    ...
    virtual float getPortVoltage(uint8_t port) { throw std::system_error(RsSdkError::FunctionNotSupported); }
	virtual float getPortCurrent(uint8_t port) { throw std::system_error(RsSdkError::FunctionNotSupported); }
