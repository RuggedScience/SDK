...
Ite8783::Ite8783(bool debug) : AbstractDioController(),
							   m_baseAddress(0)
{
	enterSio();
...
void Ite8783::setPinState(const PinConfig &config, bool state)
{
	...
	if (getPinMode(config) != ModeOutput)
		throw std::system_error(std::make_error_code(std::errc::invalid_argument), "Can't set state of pin in input mode");
