...
{
    if (portio_ioperm(data->bus, IO_MAX, 1)) {
        return -EPERM;
    }

    int status = portio_inb(HST_STS(data->bus));
    if (status & kStsBusy) {
        return -EBUSY;
    }
    ...
    if (status) {
        // Clear flags
        portio_outb(status, HST_STS(data->bus));
    }

    status = portio_inb(HST_STS(data->bus));

    // Disable CRC / PEC
    // portio_outb(portio_inb(AUX_CTL(bus)) & (~kAuxCntrlCrc), AUX_CTL(bus));
...
{
    portio_outb(kStsInUse | kStsFlags, HST_STS(data->bus));
    portio_ioperm(data->bus, IO_MAX, 0);
}

static void setHostAddress(transaction_data *data)
{
    // Assume we are given the 7-bit address instead of the 8-bit address.
    portio_outb((data->device << 0) | (data->read_write & 0x01), HST_XMIT(data->bus));
}
...
static void startTransaction(transaction_data *data)
{
    ...
    portio_outb(ctrl, HST_CTRL(data->bus));
}
...
static void handleResult(transaction_data *data, int status)
{
    ...
    if (status < 0) {
        ...
        portio_outb(kCntrlKill, HST_CTRL(data->bus));
        std::this_thread::sleep_for(sleep_time);
        portio_outb(0, HST_CTRL(data->bus));
    }
    ...
    switch (data->type) {
        ...
        case transaction_type::BYTE:
            if (data->read_write == SMBUS_WRITE)
                portio_outb(data->command, HST_CMD(data->bus));
            break;
        case transaction_type::BYTE_DATA:
            if (data->read_write == SMBUS_WRITE)
                portio_outb(data->block[0], HST_DATA0(data->bus));
            portio_outb(data->command, HST_CMD(data->bus));
            break;
        case transaction_type::WORD_DATA:
            if (data->read_write == SMBUS_WRITE) {
                portio_outb(data->block[0], HST_DATA0(data->bus));
                portio_outb(data->block[1], HST_DATA1(data->bus));
            }
            break;
        case transaction_type::BLOCK:
            portio_outb(data->command, HST_CMD(data->bus));
            if (data->read_write == SMBUS_WRITE) {
                portio_outb(data->size, HST_DATA0(data->bus));
                portio_outb(data->block[0], HST_BLK_DB(data->bus));
            }
            break;
        case transaction_type::I2C_READ:
            portio_outb(data->command, HST_DATA1(data->bus));
            portio_outb(0x00, HST_BLK_DB(data->bus));
            break;
        default:
        ...
        switch (data->type) {
            case transaction_type::WORD_DATA:
                if (data->read_write == SMBUS_READ)
                    data->block[1] = portio_inb(HST_DATA1(data->bus));
            case transaction_type::BYTE:
            case transaction_type::BYTE_DATA:
                if (data->read_write == SMBUS_READ)
                    data->block[0] = portio_inb(HST_DATA0(data->bus));
            default:
                break;
        }
    }
        ...
        for (size_t i = 0; i < data->size; i++) {
            ...
            if (data->read_write == SMBUS_READ) {
                ...
                if (data->size == SMBUS_LEN_SENTINEL) {
                    data->size = portio_inb(HST_DATA0(data->bus));
                    if (data->size < 1 || data->size > SMBUS_MAX_BLOCK_SIZE) {
                        handleResult(data, -EPROTO);
                    }
                }

                data->block[i] = portio_inb(HST_BLK_DB(data->bus));
                ...
                portio_outb(data->block[i], HST_BLK_DB(data->bus));
            }

            portio_outb(kStsDone, HST_STS(data->bus));
        }
    }
