...
float Pd69104::getPortVoltage(uint8_t port)
{
	...
	uint8_t data = portio_smbus_read_register(m_busAddr, m_devAddr, reg);
	uint16_t volts = 0x00FF & data;
	data = portio_smbus_read_register(m_busAddr, m_devAddr, reg+1);
	volts |= data << 8;
...
{
	uint8_t data = portio_smbus_read_register(m_busAddr, m_devAddr, kTotalPwrReg);
	return data;
}
...
{
	uint8_t data = portio_smbus_read_register(m_busAddr, m_devAddr, kPwrGdReg);
	if (data > 7)
		throw std::system_error(std::make_error_code(std::errc::protocol_error), "Received invalid power bank");

	data = portio_smbus_read_register(m_busAddr, m_devAddr, kPwrBankBAR + data);
	return data;
}

int Pd69104::getDeviceId() const
{
    return portio_smbus_read_register(m_busAddr, m_devAddr, kDevIdReg);
...
void Pd69104::setPortEnabled(uint8_t port, bool enabled)
{
	...
	portio_smbus_write_register(m_busAddr, m_devAddr, kPwrpbReg, data);
}

uint8_t Pd69104::getPortMode(uint8_t port) const
{
	uint8_t data = portio_smbus_read_register(m_busAddr, m_devAddr, kOpmdReg);
...
void Pd69104::setPortMode(uint8_t port, uint8_t mode)
{
	...
	portio_smbus_write_register(m_busAddr, m_devAddr, kOpmdReg, data);
}

bool Pd69104::getPortSensing(uint8_t port) const
{
	uint8_t data = portio_smbus_read_register(m_busAddr, m_devAddr, kDisenaReg);
...
void Pd69104::setPortSensing(uint8_t port, bool sense)
{
	...
	portio_smbus_write_register(m_busAddr, m_devAddr, kDisenaReg, data);
}

bool Pd69104::getPortDetection(uint8_t port) const
{
	uint8_t data = portio_smbus_read_register(m_busAddr, m_devAddr, kDetenaReg);
...
void Pd69104::setPortDetection(uint8_t port, bool detect)
{
	...
	portio_smbus_write_register(m_busAddr, m_devAddr, kDetenaReg, data);
}
